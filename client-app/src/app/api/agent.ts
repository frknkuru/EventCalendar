...
axios.interceptors.response.use(async response => {
    ...
    switch (status) {
        case 400:
            if (config.method === 'get' && Object.prototype.hasOwnProperty.call(data.errors, 'id')) router.navigate('/not-found')
            if (data.errors) {
...
const requests = {
    get: <T>(url: string, params: URLSearchParams = new URLSearchParams()) => axios.get<T>(url, { params }).then(responseBody),
...
const Profiles = {
    ...
    uploadPhoto: (file: Blob) => {
        ...
        // Use requests.post and pass formData as the body
    ...
    listActivities: (username: string, predicate: string) => requests.get<UserActivity[]>(`/profiles/${username}/activities?
    predicate=${predicate}`)
